...
export default function HomeLobbyChat({
  ...
  if (layout === "topBanner") {
    return (
      ...
      >
        <div className="mx-auto flex max-w-[1600px] flex-col gap-2 px-4 py-3 sm:flex-row sm:items-stretch sm:gap-3 sm:px-6 sm:py-4">
          <div className="min-h-0 flex-1 sm:flex sm:min-w-0 sm:flex-1 sm:flex-col">
            <p className="mb-1 text-[10px] font-medium uppercase tracking-wide text-amber-500/90 sm:shrink-0">
              Bate-papo da Sala <span className="font-normal text-amber-700">(global)</span>
            </p>
            <div
              ref={trackRef}
              className="flex max-h-16 min-h-[3rem] w-full min-w-0 items-center gap-3 overflow-x-auto overflow-y-hidden scroll-smooth rounded border border-amber-900/35 bg-stone-900/70 px-4 py-2 text-sm md:max-h-20"
            >
              {messages.length === 0 ? (
          ...
          <div className="flex w-full shrink-0 gap-2 sm:max-w-lg sm:items-end">
            <Input
              value={draft}
              ...
              maxLength={280}
              className="h-10 touch-manipulation border-amber-800 bg-stone-900 text-sm text-amber-100 placeholder:text-amber-700"
            />
            <Button type="button" onClick={send} className="h-10 touch-manipulation shrink-0 bg-amber-800 px-4 text-sm hover:bg-amber-700">
              Enviar
            </Button>
          </div>
        </div>
